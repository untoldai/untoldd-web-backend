...
export const verifyInfluncerToken = asyncHanlder(async (req, res, next) => {
    try {
        // Extract token from cookies or Authorization header
        const token = req.cookies.influncerAuthToken || req.header('Authorization')?.replace("Bearer ", "").trim();
        ...
        // Verify and decode the token
        const decodedToken = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET_MANUFACTURER);


        const admin = await User.findOne({ _id: decodedToken?._id, is_admin: false, is_influncer: true }).select("-password -refreshToken");
